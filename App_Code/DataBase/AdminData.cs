...
public class AdminData
{
    ...
    public AdminData(int Id)
    {
        ...
        using (DataSet ds = connect.GetDataset("SELECT * FROM admin WHERE id=@int_Id", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                _Password = ds.Tables[0].Rows[0]["pass"].ToString();
    ...
    public AdminData(string email, string pass)
    {
        ...
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@email", email));
        param.Add(new MySqlParameter("@pass", pass));
        using (DataSet ds = connect.GetDataset("select * from admin where email=@email and pass=@pass", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                HasValue = true;
    ...
    public void Save()
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("INSERT INTO admin(name,email,pass) VALUES(@name,@email,@pass)", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int id)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE admin SET name=@name,email=@email,mobile=@mobile,pass=@pass WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
